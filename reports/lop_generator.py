...
def generate_exhibit_cover_pdf(selected_df):
    ...
    elements.append(
        Paragraph(
            "Official TSDR Record Evidence Package — Live Registrations Used in Commerce",
            sub_style,
        )
    )
...
def run():
    ...
    st.markdown(
        "Enter goods/services manually, run a bridging search, and generate a compliant Exhibit A PDF."
    )
    ...
    st.info(
        "Check the boxes next to the specific keywords you want to cross-reference in the bridging search."
    )
    ...
    if st.button("Execute Bridging Search", type="primary"):
        ...
        if c_class == "000" or t_class == "000" or not raw_c or not raw_t:
            st.error(
                "Please enter both the Client and Applicant classes in Step 1 before searching!"
            )
            st.stop()
        ...
        if not c_selected or not a_selected:
            st.error(
                "Please check at least one box for BOTH the Client and the Applicant keywords."
            )
            st.stop()
        ...
        with st.spinner("Searching USPTO via tmsearch.uspto.gov..."):
            ...
            search_query = (
                f"GS:({t_kw_str}) AND GS:({c_kw_str}) AND IC:{c_class} AND IC:{t_class} AND LD:true"
            )
            ...
            if ui_df.empty:
                st.warning(
                    f"No bridging registrations found for query: `{search_query}`. Try checking different keyword boxes."
                )
    ...
    if st.session_state.get("lop_step") == 3:
        ...
        if results_count < 10:
            st.warning(
                f"⚠️ **Found {results_count} bridging registration(s).** A strong Letter of Protest ideally includes 5–10 solid evidence marks."
            )
            ...
            with st.expander(
                ...
                st.write(
                    "Select or enter related USPTO terms to expand your bridging search:"
                )
                ...
                if st.button("🚀 Run Expanded Search", type="secondary"):
                    ...
                    with st.spinner("Running expanded USPTO search..."):
                        ...
                        expanded_query = (
                            f"GS:({t_kw_str}) AND GS:({c_kw_str}) AND IC:{raw_c} AND IC:{raw_t} AND LD:true"
                        )
                        ...
                        if exp_df.empty:
                            ...
                            st.success(
                                f"Updated results! Total registrations found: {len(st.session_state['bridging_results'])}"
                            )
                            st.rerun()
