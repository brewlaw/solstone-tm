...
def run():
    ...
    if st.button("Generate Status Report", type="primary"):
        ...
        if ic_classes and ic_classes.strip():
        ...
        target_class_list = [c.strip().zfill(3) for c in ic_classes.split(",") if c.strip()] if (ic_classes and ic_classes.strip()) else None
        ...
        if exclude_marks and exclude_marks.strip():
        ...
        p_owner = doc.add_paragraph(f"Owner Searched:\t{owner_name}")
        if p_owner.runs: p_owner.runs[0].bold = True
        
        p_class = doc.add_paragraph(f"Class Filter:\t\t{ic_classes if ic_classes else 'All Live Classes'}")
        if p_class.runs: p_class.runs[0].bold = True
        
        p_date = doc.add_paragraph(f"Date Generated:\t{datetime.now().strftime('%B %d, %Y')}")
        if p_date.runs: p_date.runs[0].bold = True
        doc.add_paragraph()
        ...
        proper_filename_docx = f"Trademark_Report_{safe_owner}.docx"
...
<div style="overflow-x: auto; border-radius: 8px; border: 1px solid #e0e0e0;">
    {data['raw_html_table'].replace('<table border="1" class="dataframe">', '<table class="custom-table">')}
        ...
        owner_slug = str(data.get('owner_name', '')).replace(' ', '_')
        
        with dl_col1:
            st.download_button(
                label="🌐 Download HTML Report",
                data=data['html_report'],
                file_name=f"Trademark_Report_{owner_slug}.html",
                mime="text/html",
        ...
        with dl_col2:
            st.download_button(
                ...
                file_name=data.get('proper_filename_docx', 'Trademark_Report.docx'),
        ...
        with dl_col3:
            st.download_button(
                ...
                data=data['pdf_bytes'],
                file_name=data.get('proper_filename', 'Trademark_Report.pdf'),
