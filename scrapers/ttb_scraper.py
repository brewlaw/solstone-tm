...
DISCLAIMER_KEYWORDS = [
    ...
    "webmaster@ttb.treas.gov",
    "internet explorer",
]


def scrape_ttb(ttb_date_from, ttb_date_to, mark_list):
  """Scrapes TTB COLA Registry with strict ID validation and footer exclusions."""
  ttb_results = []
  seen_ttb_ids = set()
  ...
  for mark in clean_terms:
    try:
      payload = {
          ...
          "searchCriteria.brandFancifulEq": "E",
          "action": "search",
      }
      ...
      for row in rows:
        ...
        # Ignore headers, footers, and disclaimer blocks
        if any(kw in row_text.lower() for kw in DISCLAIMER_KEYWORDS):
          continue
        if (
            "TTB ID" in row_text
        ...
        # Strictly validate real TTB IDs from links or cell text
        ttb_id = None
        ...
        # STRICT RULE: Must have a verified numeric TTB ID
        if not ttb_id or ttb_id in seen_ttb_ids:
          continue
      ...
      logger.info(
          f"Scraped {variation_count} verified TTB records for '{mark}'"
      )

    except Exception as e:
      logger.warning(f"TTB HTTP error for '{mark}': {e}")
      continue
